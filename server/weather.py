...
class WeatherClient:
    ...
    def load(self, api_key):
        ...
        if "current" not in self.data:
            raise KeyError("'current' key not found in   API response")
    ...
    def hourly_summary(self, time_offset):
        ...
        dt_object = datetime.fromtimestamp(self.data["current"]["dt"])
        minute = dt_object.minute
        
        

        # Extract hour and minute
        hour = local_time.strftime("%H").lstrip("0")
        if hour == "":
            hour = "0"
        if minute == "":
            minute = "0"
    ...
    def tomorrow_summary(self):
        return self.daily_summary(1)
